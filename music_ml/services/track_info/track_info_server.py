...
import music_ml.protobuf.services.track_info.track_info_pb2 as track_info_pb2
from music_ml.protobuf.services.track_info.track_info_pb2_grpc import (
    TrackInfoServiceServicer,
    add_TrackInfoServiceServicer_to_server
)
...
class TrackInfoService(TrackInfoServiceServicer):
    def __init__(self):
        # Fetch an access token to authenticate with Spotify
        self.access_token = get_spotify_access_token()

    def GetTrackInfo(self, request: track_info_pb2.TrackInfoRequest, context):
        ...
        # Create and return a TrackInfoResponse
        track = Track(
        ...
        return track_info_pb2.TrackInfoResponse(track=track)
...
def serve():
    ...
    add_TrackInfoServiceServicer_to_server(TrackInfoService(), server)

    # Enable reflection
    SERVICE_NAMES = (
        track_info_pb2.DESCRIPTOR.services_by_name['TrackInfoService'].full_name,
    ...
    server.start()
    print("TrackInfo gRPC server is running on port 50051...")
